...
| ADR-013 | Git 提交使用仓库所有者明确提供的直接邮箱 | 已由 repo owner 确认，避免用猜测的 noreply 地址 |
...
| OPEN-10 | Teable 九表 tableId、筛选字段 fieldId 与版本差异 | P0 从 TEST 元数据导出并生成只读映射；禁止在 service 中散落 ID | Teable 管理员 | P0 |
| OPEN-11 | 临期 5 工作日、逾期 3 工作日升级、对账 5 分钟/批量 100 的正式口径 | Gate 3 固定数据集先按配置基线验证；业务负责人确认后才能启用真实调度 | 业务负责人 | Gate 4 |
| OPEN-12 | `task.cycle` 枚举、周/月/其他周期截止日及“本期已报”判定 | 原方案只明确周五 12:00 催报；业务负责人给出周期字典与样例前不实现猜测规则，先推进其余 Wave 1/2 卡 | 业务负责人 | Gate 3 |
| OPEN-13 | 审读低置信度阈值 0.6 与真实模型质量 | Gate 3 离线结构按 0.6 配置基线验证；EXT-01 到位后以至少 200 条双人标注集复核 precision/recall 并签字定案 | 业务负责人/AI 平台 | Gate 4 |
