...
AI 离线配置：`LLM_MODE=disabled|mock`、`AI_REVIEW_CONFIDENCE_THRESHOLD=0.6`、`LLM_TIMEOUT_S=20`、`LLM_MAX_CONCURRENCY=4`。mock 只供 Wave 2/Gate 3 结构闭环且生产强制拒绝；`LLM_BASE_URL/API_KEY/MODEL` 在 EXT-01 POC 前不得解释为已联通。模型输入作为独立规范 JSON 传给 adapter，不拼进 system prompt；原始模型异常正文不得进入日志或 `ai_run`。
...
管理端触发同样禁止在 `sd-api` 内执行。API 事务写 `job_trigger_request` 与 `scheduler.run_job` outbox，worker 消费后进入同一 `SchedulerService`；任务失败已落 `job_run=failed` 后视为命令已消费，不做隐式自动重跑，必须由管理员显式引用失败 run 发起新请求。这样避免外呼结果未知或业务副作用被 outbox 自动重放。
