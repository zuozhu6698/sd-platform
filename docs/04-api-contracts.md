...
| 15 | `/api/admin/runs/{job}/trigger` | POST | `ops_admin/supervision_admin`；手动触发或失败补跑，需 CSRF、幂等键和审计，HTTP 202 |
...
`GET /api/admin/runs?cursor=<job_run_id>&limit=25` 按 `started_at/job_run_id` 倒序返回 `job_run_id/job/scheduled_for/state/counts/error_code/started_at/finished_at`，不返回任务输入正文或外部凭据。

`POST /api/admin/runs/{job}/trigger` Header 必须包含 UUID `Idempotency-Key` 和有效 `X-CSRF-Token`。首次手动触发 body 为 `{}`；失败补跑 body 为：

```json
{"retry_of_job_run_id":"22222222-2222-4222-8222-222222222222"}
```

成功仅表示 durable command 已入队，返回 HTTP 202：

```json
{"data":{"trigger_id":"...","outbox_id":"...","job":"urge_scan","scheduled_for":"2026-07-24T12:00:00+08:00","retry_of_job_run_id":null,"state":"queued","idempotent":false},"request_id":"req_..."}
```
...
审读 handler 只处理 `ai_run_id` 为空且 `submitted_at <= scheduled_for` 的流水，按提交时间/`log_id` 排序并限制批量；上下文只取所属事项、重点工作目标、当前填报和同事项最近四期历史。通过写 `review_status=ai_passed`，标记写 `review_status=pending_confirmation`；同时写 `ai_result/ai_comment/ai_question/ai_run_id`。`ai_comment` 是 flags/evidence/confidence/source_ids 的规范 JSON，不保存模型自由格式正文。
...
Wave 1 固化两类 durable OA 命令：`complete_pending(command_id, task_id, person_id, log_id, dedup_key)` 与 `send_urge(task_id, target_id, level, content, planned_at, dedup_key)`。OA mock 对两者均能复现同键冲突、429、5xx、业务拒绝及“服务端已接收但客户端超时”；催办成功后以同一 dedup key 幂等追加 `urge_log` 回执。真实接口认证、端点和字段映射保持 `pending-EXT-03`，不得从 mock 推断厂商 contract。
