...
更新日期：2026-07-24
状态：`CORE_FOUNDATIONS_IN_PROGRESS`，工程基线、会话鉴权端点、本人任务、规则引擎、Teable adapter、幂等填报 saga、安全附件链路、durable outbox worker、dead letter 双人审批与幂等补发已验证；SSO 回调、OA 分发和外部 POC 未完成，不允许生产部署。
...
| 代码/测试/CI | FastAPI/Vue/Alembic/Compose/CI 已创建；PR #5 已经受保护主干合并，合并后 GitHub Actions run `30059161949` 四个 job 全绿 | changed-and-verified-remote |
...
| 工作区 | changed-and-verified | PR #5 四项 CI 全绿后 squash merge；`main`=`30342d4` 且受保护；B6 补发能力已进入主干 |
