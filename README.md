...
- 工程门禁：后端 Ruff/format/Mypy/236 tests（总覆盖率 90.26%，纯规则模块 100%）通过；前端 lint/typecheck/5 tests/build（被测模块四项覆盖率 100%）通过；文档链接与 secret shape 检查通过。
- 核心底座：JWT/CSRF/重定向防护、PostgreSQL session 撤销、动态角色范围、`/api/me`、`/api/logout`、纯规则引擎、Teable 九表字段白名单与安全重试适配器、`/api/report/submit` 幂等 saga、附件扫描状态约束和 durable outbox 核心已实现；OA start/callback、OA 分发、文件上传/扫描与真实 Teable/OA 联调仍以 POC 为准。
